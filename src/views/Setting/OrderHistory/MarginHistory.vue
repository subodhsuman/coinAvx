            ...
            <div class="setting_slot_box">
                ...
                <div class="setting_inner_content row  justify-content-between p-md-5 p-2">
                    ...
                    <div class="col-md-12 col-lg-12 col-xl-12">
                        <div class="m-history-table table-responsive">
                            <table class="table table-borderless align-middle">
                                <thead>
                                    <tr class="text-center">
                                        <th scope="col" class="ps-5 text-start">Date</th>
                                ...
                                <tbody v-if="loading">
                                    <template v-if="data_found">
                                        <tr class="text-center" v-for="(data,index) in Margin_HistoryData" :key="index">
                                            <td class="py-3 ps-5 text-uppercase text-start"> {{data.date}}  </td>
                                            <td class="py-3 text-capitalize">{{data.pairs}} </td>
                                            <td class="py-3">{{data.type}}</td>
                                            <td class="py-3">{{data.side}}</td>
                                            <td class="py-3">{{data.average}} </td>
                                            <td class="py-3">{{data.price}} </td>
                                            <td class="py-3">{{data.executed}} </td>
...
export default {
    ...
    data() {
        return {
            ...
            perPageData: 1,
            loading: true,
            Margin_HistoryData: PortfolioData.Margin_HistoryData
...
.m-history-table thead tr th:first-child {
    border-top-left-radius: 20px;
    border-bottom-left-radius: 20px;
}

.m-history-table thead tr th:last-child {
    border-top-right-radius: 20px;
    border-bottom-right-radius: 20px;
}

.m-history-table thead tr th {
    font-weight: 500;
    font-size: 14px;
}

.m-history-table tbody tr td {
    font-weight: 500;
    font-size: 14px;
}

.m-history-table .form-check-input:checked {
    background-color: var(--avx-yellow);
    border-color: var(--avx-yellow);
}

.m-history-table .form-check-input {
